...
class ColumnReorderTest < ApplicationSystemTestCase
  ...
  test "dragging a column header reorders columns and persists globally" do
  ...
  test "column order comes from the server after reload, not localStorage" do
    drag_col("PG-2", "PG-1", side: :left)
    wait_for_saved_order %w[PG-2 PG-1]
  ...
  test "column order is preserved after a sync broadcast" do
    drag_col("PG-2", "PG-1", side: :left)
    wait_for_saved_order %w[PG-2 PG-1]

    Issue.create!(
      jira_key: "PG-77", epic: epics(:priority_one), summary: "Broadcast marker",
      jira_status: "In Progress", issue_type: "Task",
      created_at_jira: Time.current, status_changed_at_jira: Time.current
    )
    broadcast_board

    assert_selector "[data-tooltip-id='PG-77']", visible: :all
    assert_selector ".kb-col:first-child[data-epic-key='PG-2']"
    assert_equal %w[PG-2 PG-1], column_keys
  end

  test "another session sees the reordered columns" do
    drag_col("PG-2", "PG-1", side: :left)
    wait_for_saved_order %w[PG-2 PG-1]
