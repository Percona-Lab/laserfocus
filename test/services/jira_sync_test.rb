...
class JiraSyncTest < ActiveSupport::TestCase
  ...
  def collapsing_sync
    JiraSync.new(
      epic_query: 'project = PG AND labels = "Priority"', unplanned_query: nil,
      status_map: { "To Do" => "new", "In Progress" => "in_progress" },
      new_statuses: [ "new" ]
    )
  end

  test "collapses a new epic whose status maps to new" do
    stub_epic_search("To Do")
  ...
  test "does not collapse a new epic in progress" do
    stub_epic_search("In Progress")

    collapsing_sync.run!

    assert_equal [], BoardOrder.instance.collapsed_columns
  end

  test "does not collapse an existing epic that moves to a new status" do
    stub_epic_search("In Progress")
    collapsing_sync.run!
    assert_equal [], BoardOrder.instance.collapsed_columns
  ...
  test "does not duplicate the key when a collapsed epic is re-added" do
    BoardOrder.instance.update!(collapsed_columns: [ "PG-1" ])
    Epic.create!(jira_key: "PG-1", name: "Epic A", jira_status: "To Do", priority: 1, removed_at: 1.day.ago)
    stub_epic_search("To Do")
  ...
  test "collapses a re-added epic in a new status even if it was expanded before" do
    Epic.create!(jira_key: "PG-1", name: "Epic A", jira_status: "To Do", priority: 1, removed_at: 1.day.ago)
    stub_epic_search("To Do")
